...
public class Main {
  ...
  private static AsyncOperation<String> getAsyncOp(int index) {
    return AsyncGraph.createAsync(() -> longRunningOp(index))
        .check(
            num ->
                System.out.println(
                    String.format("Thread %d: long op returned %d", getThreadId(), num)))
  ...
  public static void main(String[] args) {
    ...
    AsyncOperation<String> asyncOp3 =
        AsyncGraph.createImmediateAsync(3).thenCollapse(num -> getAsyncOp(num));
    AsyncOperation<Integer> asyncOp4 =
        AsyncGraph.createImmediateAsync(4)
            .consume(num -> {})
            .check(
                v -> System.out.println(String.format("Thread %d: Value is %s", getThreadId(), v)))
            .then(v -> 4);
    ...
    AsyncOperation<String> asyncOpFinal =
        AsyncGraph.createCombinedAsync(
            ...
            asyncOp4,
            (str1, str2, str3, int4) ->
                String.join(", ", str1, str2, str3, Integer.toString(int4)));
