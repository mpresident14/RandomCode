...
int knapsack_helper(
    int* arr, int* weights, size_t length, int limit, int index,
    unordered_map<pair<int, int>, int, boost::hash<pair<int, int>>>& memo) {
  if (index == 0) {
    return 0;
  }

  // Memoization: save previously calcuated results -- (index, limit) :
  // best_value
  ...
  if (wt > limit) {
    ...
    int useit = arr[index - 1] + knapsack_helper(arr, weights, length,
                                                 limit - wt, index - 1, memo);
