...
void getCombosHelper(int* array, int* combo, int length, int comboSize,
                     int distanceToEnd, int arrayIndex, int comboIndex) {
  if (comboIndex == comboSize) {
    printArray(combo, comboSize);
  }

  else {
    for (size_t i = arrayIndex; i < 2 * length - comboSize - distanceToEnd + 1;
         ++i) {
      combo[comboIndex] = array[i];
      getCombosHelper(array, combo, length, comboSize, distanceToEnd - 1, i + 1,
                      comboIndex + 1);
    }
  }
}
...
void findSumCombosHelper(int* array, int* combo, int length, int comboSize,
                         int target, int distanceToEnd, int arrayIndex,
                         int comboIndex, int currentSum) {
  if (comboIndex == comboSize) {
    ...
    for (size_t i = arrayIndex; i < 2 * length - comboSize - distanceToEnd + 1;
      ...
      findSumCombosHelper(array, combo, length, comboSize, target,
                          distanceToEnd - 1, i + 1, comboIndex + 1,
                          currentSum + array[i]);
    }
  }
}
