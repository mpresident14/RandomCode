...
class Thing {
	public:
		// w is const, so move(w) creates a const Widget rvalue. The constness prevents 
		// it from being passed to the move constructor. However, the copy constructor 
		// is happy to accept it.
		Thing(const Widget w)
			:w_{move(w)}
		{};
