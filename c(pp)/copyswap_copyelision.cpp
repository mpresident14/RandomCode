...
class Practice {
    ...
    public:
        ...
        //     std::cout << "&other=" << &other << std::endl;
        //     std::cout << "Assignment Operator" << std::endl;
        //     // Passing by reference forces us to create a tmp object before swapping so that we 
        //     // do not invalidate other. This forces a call to the copy constructor.
        //     // Passing by value in the function above allows the compiler to use copy elision 
        //     // when possible, whereas this version prevents this opportunity for optimization.
        //     Practice tmp{other}; 
        //     swap(*this, tmp);
        //     return *this;
        // };
        ...
        // The local variable variable prac is created directly in the parent stack frame,
        // rather than in its own return frame. This "elides" the temporary copy. Also 
        // known as (named) return value optimization.
        Practice operator+(Practice& other)
        {
        ...
        // In a situation with multiple local vars that can possibly be returned, copy elision
        // is difficult for the compiler. In this case, the compiler is required to treat 
        // returning a local var as if it were written "return std::move(local_var)". Thus,
        // it calls the move constructor.
...
// Move constructor         * Compiler must treat as if "return std::move(local2)"
// Destructor 0             * Move Constructor: swap if uninitialized Practice
// Destructor 15            * Destroy local1
