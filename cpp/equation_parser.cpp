...
bool isInt(char c) { return c >= 48 && c <= 57; }
...
bool isLowerLetter(char c) { return c >= 97 && c <= 122; }
...
double evaluate(string eq, char var, double val) {
  ...
  while (i < length) {
    ...
    // If the dependent variable (e.g. x or n) is the next char
      ...
      nextNum = evaluate(getEqInParens(eq, i), var,
                         val);  // This fcn advances i to correct position
