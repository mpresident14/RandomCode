...
public class OneTimePadBreaker {
  ...
  /** XORs the cipher at <cipherIndex> with every other cipher and returns the results. */
  ...
  /**
   * Finds each i where every xoredMessages[i] ^ c == a valid letter. Since each xoredMessage was
   * xored with the same cipher, we know that the message corresponding to that cipher has char c at
   * index i.
   */
  ...
  private static boolean isValidLetterOrSpace(byte c) {
    return OTHER_SYMBOLS.chars().anyMatch(ch -> ch == c)
        || c == 0x28
        || (c >= 'A' && c <= 'Z')
        || (c >= 'a' && c <= 'z');
  }
